...
var DB_PATH = join(DATA_DIR, "media.db");
var UPLOADS_DIR = join(DATA_DIR, "uploads");
...
app.post("/", async (c) => {
  ...
  const id = uuidv7();
  const stmt = db.query("INSERT INTO folders (id, name, parent_id) VALUES (?, ?, ?) RETURNING *");
  const row = stmt.get(id, body.name.trim(), body.parent_id ?? null);
  return c.json(row, 201);
});
app.get("/:id", (c) => {
  const id = c.req.param("id");
  const row = db.query("SELECT * FROM folders WHERE id = ?").get(id);
  if (!row)
...
async function saveFile(file, filename) {
  const path = join2(UPLOADS_DIR, filename);
...
app2.post("/", async (c) => {
  ...
  const stmt = db.query(`INSERT INTO media
     (id, filename, original_name, mime_type, size, folder_id, storage_path, media_type)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     RETURNING *`);
  const row = stmt.get(id, filename, file.name, file.type || "application/octet-stream", size, folderId, storagePath, mediaType);
  return c.json(row, 201);
});
app2.get("/:id", (c) => {
  const id = c.req.param("id");
  const row = db.query("SELECT * FROM media WHERE id = ?").get(id);
  if (!row)
    return c.json({ error: "not found" }, 404);
  return c.json(row);
});
app2.get("/:id/download", async (c) => {
  const id = c.req.param("id");
  const row = db.query("SELECT * FROM media WHERE id = ?").get(id);
  if (!row)
    return c.json({ error: "not found" }, 404);
...
app2.delete("/:id", (c) => {
  const id = c.req.param("id");
  const row = db.query("SELECT storage_path FROM media WHERE id = ?").get(id);
  if (!row)
    return c.json({ error: "not found" }, 404);
...
app3.use(cors());
app3.use(logger());
app3.route("/folders", folders_default);
app3.route("/media", media_default);
app3.get("/health", (c) => c.json({ status: "ok", db: db.filename }));
